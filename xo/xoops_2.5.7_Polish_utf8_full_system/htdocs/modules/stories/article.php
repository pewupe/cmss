...
if (isset($HTTP_COOKIE_VARS[$lvcid])) {
	$lastviewed = intval($HTTP_COOKIE_VARS[$lvcid]);
}
if ($xoopsUser) {
	...
	setcookie($lvcid,strval($newlv),time()+2592000);
} else {
	setcookie($lvcid,strval($newlv),time()+2592000);
	$lastviewed = time()+2592000;
}
$xoopsOption['template_main'] = 'stories_article.html';
...
include 'stories_comment_view.php';
