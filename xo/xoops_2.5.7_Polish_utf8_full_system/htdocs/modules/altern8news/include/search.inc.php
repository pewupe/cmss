<?php
// $Id: search.inc.php,v 1.1 2003/10/14 06:23:17 mikhail Exp $
//  ------------------------------------------------------------------------ //
...
function altern8news_search($queryarray, $andor, $limit, $offset, $userid){
	global $xoopsDB;
	$sql = "SELECT storyid,uid,title,created FROM ".$xoopsDB->prefix("atikel")." WHERE published>0 AND published<=".time()."";
	if ( $userid != 0 ) {
		$sql .= " AND uid=".$userid." ";
	}
